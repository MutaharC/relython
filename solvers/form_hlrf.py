...
def hlrf(g, xdists, u_to_x, T, inp):
    ...
    u0 = zeros(len(inp['vars']))
    d_beta = 999
    u = u0
    beta_arr = zeros(inp['maxitr'])
    ...
    for i in range(inp['maxitr']):
        ...
        grad = dgdu(g, u, u_to_x, xdists, T, inp['eps'])
        ...
        if d_beta <= inp['tol'] and abs(gu) <= inp['ftol']:
            break
    ...
    return {'vars': xdists, 'beta': beta, 'Pf': Pf, 'u_beta': u, 'x_beta': x,
            'alpha': alpha, 'nitr': i+1, 'g_beta': gu, 'msgs': msgs}
